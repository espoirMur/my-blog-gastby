...
`mkdir read_emails_project & cd read_emails_project`{:.language-clojure .highlight}

Once in the project, let’s initialize git :

`git init`{:.language-clojure .highlight}

Once we initialized git, let’s create a virtual environment:
`virtualenv -p python .venv`{:.language-clojure .highlight}

You can activate the virtual environment with the following line of code :

`source .venv/bin/activate`{:.language-clojure .highlight}
...
We will be using `.env`{:.language-clojure .highlight} file, whose content will be known to us only, and we will never share it with anyone online, even putting it in a private git repository.

Let us create the `.env`{:.language-clojure .highlight} file 

`touch .env`{:.language-clojure .highlight}

And create a gitignore file to tell git which files to ignore.

`touch .gitignore`{:.language-clojure .highlight-clojure .highlight}

open the .gitignore and add the following lines  it to ignore the `.env`{:.language-clojure .highlight} file :
```
    .venv
    env
```
In those lines, we are instructing git to ignore our `.env`{:.language-clojure .highlight} files and our virtual environments.

Now we can populate the `.env`{:.language-clojure .highlight} file with our credentials :

```
USER_EMAIL='your@email.com'
USER_PASSWORD='your secret password.'
```
We have recorded  our email credentials in the `.env`{:.language-clojure .highlight} file; let us now write a method that reads them from there.
...
`pip install -U python-dotenv`{:.language-clojure .highlight}

let us write a function that reads and returns those environment variables : 

You can create a file with a name `utils.py`{:.language-clojure .highlight} and add the following lines of code : 
...
From that function, you can notice that we are using the `load_env` function, it loads the environment variables from our `.env`{:.language-clojure .highlight} file and exposes them to system environment variables so we can be able to read them from there.
...
I was happy to find that Python has a built-in feature that enables us to connect to a mailbox. It is in the `imaplib`{:.language-clojure .highlight} module.
It also has a module to parse emails.

Let's create a file `reading_emails_scripts.py`{:.language-clojure .highlight} inside it; we are about to perform all the magic we need.
...
create a file called `run.py`{:.language-clojure .highlight} and add the following code inside: 
